...
#### 获取插件
想要获取最新当前最新版本，请在此处下载：[WarningCore](https://github.com/lunzhiPenxil/OlivaDiceDocs/raw/master/_release/Warning%E8%BD%AC%E5%8F%91%E6%9C%BAv1.1.4.epk)    
...
- 创建`RadioData/warningconf.ini`*（可选）*
    在`RadioData`文件夹内创建`warningconf.ini`文件并写入如下内容    
    ```ini
    [conf]
    time=30
    ```
    即可将时间戳校验的阈值调整为`30`秒，当没有设置时该设置默认为`30`。
