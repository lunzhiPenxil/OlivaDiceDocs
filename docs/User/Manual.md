...
## 前言
**“骰子机器人”**的本质，是一个通过或非官方（QQ）协议以及工具，或官方（Telegram、KOOK等）开放平台接口，实现的一个部分或者全部由骰子机器人软件及其控制的普通用户账号或特殊账号的集合体，而“骰子”一词则，将该集合体的目标功能，限定为服务发生于这些场景的跑团中，提供模拟掷骰的功能以及其他与跑团相关的功能。  
...
`.help` 查看默认帮助  
`.help [查询内容]` 查询对应的帮助内容  
`.help 全牌堆列表` 查询骰子机器人所加载的全部牌堆  
`.help 指令` 查看指令列表  
`.help 管理` 查看管理指令  
`.help 手册` 查看完整手册  
`.help 链接` 查看源码/相关文档链接  
`.help 频道` 查看频道(Host)相关帮助  
...
**神话淬炼机制说明：**  
此为可选规则。当调查员的克苏鲁神话技能点数超过理智值(SAN)时，他将经历范式转移。  
...
默认情况下，`.sn` 会自动判断群名片还是全局名片  
使用 `.sn group` 可显式设置当前群的名片  
使用 `.sn global` 可设置全局名片，并清除当前群的专属名片  
群名片优先级高于全局名片  
`group/global` 可与格式参数组合，如: `.sn group coc`, `.sn global template`  
...
## 房规设置
`.settemp [规则名]` 为当前群或频道组设置不同的规则  
`.setrule [村规名]` 为当前群或频道组设置村规  
`.setcoc [参数]` 为当前群或频道组设置COC房规  
`.setrav [参数]` 为当前群或频道组设置RAV判定规则  
`.setdnd` 切换当前群或频道组设置DND5E房规开启状态  

设定房规后，本群录入人物卡时，会自动使用所设定的房规  
...
`.log quote (名字)` 引用日志(平台需为QQ)  
`.log quote on/off` 开启/关闭自动引用上次结束日志功能  
`.log pcname on/off` 开启/关闭日志使用角色卡名字功能  
开启后日志记录时将使用发送者的角色卡名字  
`.log stat (UUID) (all/@用户)` 查看日志统计数据  
不带参数时，查看当前活跃日志中自己的数据  
