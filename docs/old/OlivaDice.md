...
#### 跑团
尽管对这个会有多种定义，但总得来说在该项目所属社区内，不会超出其作为一个TRPG（Tabletop Role Playing Game）子集的范畴，所以作为一种G（Game），它当然是一种娱乐，除非你对娱乐有其它特别定义（比如当用这个词特指除跑团以外的娱乐内容时，需注意这个情形通常被一部分社区成员默认，所以建议双方不要浪费精力在无意义的定义纠结上）。
#### 服务器
