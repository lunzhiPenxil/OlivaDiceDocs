...
如果你对本页面内容都没有价值观性质的疑问的话，欢迎加入仑质的青果用户群讨论本分支版本相关内容，后续如果有必要的话将会单独分群。    
- 青果批发车间 `828786809`
...
#### 简介
在`Shiki`分支中，设置心跳上报与否的配置项不存在，相关逻辑为**强制启用**，且所有的上报服务器路径、端口、内容都是直接硬编码写死在相关代码中的，不存在鉴权，对于一个使用AGPL协议的开源项目而言，这是非常愚蠢的。
...
| isVisible                     | 是否愿意可见                    | 与`CloudVisible`相反            |
...
当0x010为真时，选择对自定义的心跳服务器进行上报，默认为指向仑质的心跳服务器，但除非设置了**有效的Token**，否则**不会上报**。
